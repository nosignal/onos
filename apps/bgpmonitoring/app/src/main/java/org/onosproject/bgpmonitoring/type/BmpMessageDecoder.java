...
public class BmpMessageDecoder extends FrameDecoder {
    ...
    private static final Logger log = LoggerFactory.getLogger(BmpMessageDecoder.class);
    ...
    protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws Exception {
        ...
        try {
            if (buffer.readableBytes() > 0) {
                ...
                return (BmpPacket) BmpHeader.deserializer().deserialize(bytes, 0, bytes.length);
            }
        } catch (Exception e) {
            log.error("BMP message decode error ", e);
